...
💡 **プライベート管理のコツ**: 全体チャンネルの重要なメッセージは「転送」で自分のDMに送ってからリアクションすれば、誰にも見られずにTodo管理できます！
...
全体チャンネルで目立たないように、Slackの転送機能を使うのがおすすめです：

1. **重要なメッセージを右クリック → 「転送」**
2. **自分宛のDM（自分自身）に転送**
3. **転送したメッセージに 👀 リアクションを付ける**
4. **CLIで確認**
   ```bash
   todo list  # 自動的に同期される
   ```
...
もちろん、どのチャンネルのメッセージでも直接👀リアクションを付けてもOKです。
